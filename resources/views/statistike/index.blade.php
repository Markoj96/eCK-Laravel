            ...
            <div class="container-fluid">
                ...
                <div class="row">
					<div class="col col-lg-12">
                        @php
                            if(!$ukupnoVolontera) $ukupnoVolontera = 0;
                            if(!$muskihVolontera) $muskihVolontera = 0;
                            if(!$zenskihVolontera) $zenskihVolontera = 0;
                            if(!$ukupnoAktivnosti) $ukupnoAktivnosti = 0;
                            if(!$trenutnoOtvorenih) $trenutnoOtvorenih = 0;
                            if(!$prosecanBroj) $prosecanBroj = 0;
                        @endphp
                        <div class="jumbotron">
                            <h2> Volonteri </h2>
                            
                            <p>Broj volontera: {{$ukupnoVolontera }}</p>
                            <p>Broj muskih volontera: {{$muskihVolontera }}</p>
                            <p>Broj zenskih volontera: {{$zenskihVolontera }}</p>
                        </div>
                        <div class="jumbotron">
                            <h2> Aktivnosti </h2>

                            <p>Ukupno aktivnosti: {{$ukupnoAktivnosti }}</p>
                            <p>Trenutno otvorenih aktivnosti: {{$trenutnoOtvorenih }}</p>
                        </div>
                        <div class="jumbotron">
                            <h2> Seminari </h2>
                            
                            <p>Trenutno otvorenih seminara: {{$trenutnoOtvorenih }}</p>
                            <p>Prosecan broj ucesnika: {{$prosecanBroj }}</p>
                        </div>
    				</div>
                </div>
                @endif
